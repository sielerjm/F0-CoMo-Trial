...
ps.all <- readRDS(paste0(objects.path, "/phyloseq_sampData_cleaned_NoOutliers_2022-02-13.rds"))
...
df.all$Exposure <- factor(df.all$Exposure, levels=c("Unexp", "Exp"))
dt.all$Exposure <- factor(df.all$Exposure, levels=c("Unexp", "Exp"))
...
# Dataframe
df.expFin <- df.all %>%
  filter(Timepoint == "6mpf" & Exposure == "Exp")
...
# Phyloseq Object
ps.expFin <- ps.all %>%
  subset_samples(Timepoint == "6mpf" & Exposure == "Exp")
