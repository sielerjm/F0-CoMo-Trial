...
gen_glm_anova <- function(tmp.mod, tmp.metric){
...
melt_to_datatable <- function(alpha.base.norm, methods, var.name){
  model.data <- sample.dt[alpha.base.norm, on = ("Sample")] %>% setkeyv("Sample")
  return( melt(
            model.data,
...
gen_bar_plot <- function(data, x.var, y.var = "Count", facet.var.x = ".", facet.var.y = ".", colors = pal.dark2, title = "", caption = ""){
...
gen_PCoA_Cat_Plot_2 <- function(data, x.var, y.var, axis, stats, x.var.2 = "", 
                                elps.var = x.var, title = "", caption = "", disp.stats = F,
                                colors = pal.dark2){
  ...
  p <- {lapply(names(y.var), function(beta){
    ...
    tmp.p[[beta]] <- ggplot(data[Dist == beta], aes(x = CAP1, y = data[Dist == beta][[3]])) +
      ...
      labs(
        ...
        title = ifelse(title == "", paste0("Beta.Score ~ ", paste0(c(x.var, x.var.2), collapse = " + ")), title),
...
betaUncon_plot_num <- function(data, p.method = "PCoA", physeq, dist = c("bray", "canberra", "sor"), 
                           facet.var.x = ".", facet.var.y = ".", tmp.ellipse = NULL,
                           x.var = NULL, x.var.2 = "", title = "", colors = "Spectral"){
  ...
  p <- {lapply(dist, function(beta){
    ...
    tmp.p[[beta]] <- plot_ordination(
      ...
      geom_point(aes(fill = eval(parse(text = data$x.var)),
                     color = eval(parse(text = data$x.var)),
                     shape = eval(parse(text = x.var.2)),
                     size = eval(parse(text = data$x.var))),
                 alpha = .9) +
      scale_size_area() +
      stat_ellipse(aes(linetype = eval(parse(text = tmp.ellipse)) )) +
      # geom_label_repel(data = calc_Centroid_2(tmp.ord, x.var = x.var),
      ...
      #                  alpha = 0.80) +
      facet_grid(paste0(facet.var.y, " ~ ", facet.var.x), scales = "free_y") +
      scale_color_distiller(palette = colors) +
      # scale_fill_manual(values = colors) +
      # scale_color_manual(values = colors) +
      labs(
        # title = ifelse(title != "", title, paste0(c(p.method, beta), collapse = ", "))
...
# betaCon_plot <- function(data, p.method = "PCoA", physeq, dist = c("bray", "canberra", "sor"), 
#                            facet.var.x = ".", facet.var.y = ".", 
#                            x.var = NULL, x.var.2 = NULL, shape = NULL, title = ""){
#   
#   
#   p <- {lapply(dist, function(beta){
#     
...
plot_diffAbund_2 <- function(data, x.var="Phylum", fill.var="coef"){
  ...
  ggplot(aes(x=fct_rev(eval(parse(text = x.var)) ), 
    ...
    geom_bar(aes(fill = Genus ), 
             position = position_dodge2(width = 0.9, preserve = "single"), 
             size = ifelse(length(unique(data$Genus)) < 8,1,.25) ) + 
    
    facet_wrap(.~value, scale="free", ncol = 1) +
    scale_fill_manual(values = c(pal.dark2, pal.paired)) +
...
diffAbund_Maaslin2 <- function(physeq,
                               ...
                               min.abundance = 0.1, #0.0001,  # Conservative threshold 
  ...
  tmp.obj[["Parameters"]] <- #tryCatch({
    # Insert Code
                                list(physeq.ID = physeq.ID,  # Phyloseq ID ("ps.T0")
